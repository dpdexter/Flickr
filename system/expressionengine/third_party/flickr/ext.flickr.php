...
class Flickr_ext {
	...
	var $name           = FLICKR_NAME;
	var $version        = FLICKR_VER;
	var $description    = FLICKR_DESC;
	var $docs_url       = FLICKR_DOCS;
	...
	function settings_form($current)
	{
		...
		switch(($this->settings['option_auth'] != '') ? 'ok' : 'setup') :
			...
			case('setup') :
				...
				// config callback_url
				$vars['callback_url'] = 'http://' . $_SERVER['HTTP_HOST']. $_SERVER['PHP_SELF'] . '?D=cp&C=addons_extensions&M=extension_settings&file=flickr';
	...
	function _migrate_from_ft()
	{
		...
		$settings = $this->EE->api_channel_fields->get_global_settings('flickr');
...
/* End of file ext.flickr.php */ 
/* Location: ./system/expressionengine/third_party/flickr/ext.flickr.php */