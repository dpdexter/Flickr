...
class Flickr_helper {
	...
	function __construct()
	{
		...
		if( ! isset($this->EE->session->cache['flickr']))
		{
			$this->EE->session->cache['flickr'] = array();
		}
		$this->cache =& $this->EE->session->cache['flickr'];
	}
	// END
	...
	{	
		...
		if( ! isset($this->cache['cp_js']))
		{
			...
			$this->EE->cp->add_to_head('<script type="text/javascript">flickr = {}; flickr.browserUrl = "' . $this->browser_url('wygwam') . '";</script>');
	...
	function theme_url()
	{
		if( ! isset($this->cache['theme_url']))
		{
			...
			$this->cache['theme_url'] = $this->EE->functions->remove_double_slashes($theme_folder_url . '/third_party/flickr/');
		}
...
/* Location: ./system/expressionengine/third_party/flickr/helper.php */